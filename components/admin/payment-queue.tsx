...
import { confirmPayment, rejectPayment } from "@/lib/actions/payment";
...
export function PaymentQueue({ rows }: { rows: PaymentRow[] }) {
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                disabled={pending}
                onClick={() => run(() => confirmPayment(p.id))}
              >
                {t("confirm")}
              </Button>
              <Button
          ...
          {rejecting === p.id ? (
            <div className="mt-3 flex items-center gap-2">
              ...
              <Button
                ...
                className="text-destructive"
                disabled={pending}
                onClick={() => run(() => rejectPayment(p.id, reason))}
              >
                {t("confirmReject")}
              </Button>
            </div>

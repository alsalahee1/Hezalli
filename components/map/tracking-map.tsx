...
// Live courier position on the public tracking page. Polls the location
// endpoint every 20s and moves the driver marker. Renders nothing until the
// server actually reports a driver point (i.e. the parcel is out for delivery
// and the courier has shared location). Leaflet is imported lazily to dodge SSR.
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (!res.ok) return;
        const data = (await res.json()) as LocResp;
        ...
        // Destination marker (static, drawn once) + fit both in view.
        if (data.dest) {
          leaflet
